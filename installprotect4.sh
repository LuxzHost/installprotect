...
class NodeController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        $user = Auth::user();
        if (!$user || $user->id !== 1) {
            abort(403, '🚫 Akses ditolak! Hanya admin ID 1 yang dapat membuka menu Nodes. ©Protect By Luxz V2.3');
        }
        // ======================================================
...
sed -i "s|Protect By Luxz|${BRAND_TEXT}|g" "$REMOTE_PATH" 2>/dev/null || true
